...
* [Fonts](/codisredding/dotfiles/tree/master/conf/osx/fonts) via the [init/50_osx_fonts.sh](init/50_osx_fonts.sh) script
...
If you modify things and notice a bug or an improvement, [file an issue](https://github.com/codisredding/dotfiles/issues) or [a pull request](https://github.com/codisredding/dotfiles/pulls) and let me know.
...
You might want to set up your ubuntu server [like I do it](https://github.com/codisredding/dotfiles/wiki/ubuntu-setup), but then again, you might not.
...
1. Fork this repo
1. Open a terminal/shell and do this (change `codisredding` and `master` as appropriate):

#### Ubuntu

```sh
export DOTFILES_GH_USER=codisredding
...
```sh
bash -c "$(wget -qO- https://bit.ly/codisredding-dotfiles)" && source ~/.bashrc
```

#### macOS

```sh
bash -c "$(curl -fsSL https://bit.ly/codisredding-dotfiles)" && source ~/.bashrc
```
